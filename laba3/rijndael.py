class ECB():
    ...
    def encode(self, b_arr):
        ...
        blocks[-1] = int.from_bytes(blocks[-1],
                                    byteorder='little').to_bytes(self.bs, byteorder='little')

        res = bytearray()
        for block in blocks:
            res.extend(self._aes.encode_block(block))
        return res
    ...
    def decode(self, b_arr):
        ...
        res = bytearray()
        for block in blocks:
            res.extend(self._aes.decode_block(block))
...
class Rijndael:
    def __init__(self, key, block_len):
        '''
        key_len in {4, 6, 8} (128 or 192 or 256 bits)
        block_len in {4, 6, 8}
        '''
    ...
    def key_expansion(self, key):
        ...
        # transform to 2d array (4 rows, Nk columns)
    ...
    def mix_columns(self, state, dec=False):
        ...
        for i in range(self.Nb):
            ...
            for j in range(4):
                state[j][i] = mul(c[0], st[0]) ^ mul(
                    c[1], st[1]) ^ mul(c[2], st[2]) ^ mul(c[3], st[3])
...
if __name__ == "__main__":
    rdl = Rijndael('1234567890abcdef'.encode(), 4)
    mg = CFB(rdl, 'asdn123412341234'.encode())
